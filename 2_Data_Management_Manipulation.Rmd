...
### Talk about metadata and why scripting data cleaning/management is a _great_ idea. 

- good metadata - your most important collaborator is yourself six months ago, so leave yourself good notes, since you can't send yourself six months ago an email. You and anyone who uses your data/code later on with thank you. 
...
- read your data into R
    ...
    - IF you can get all the spreadsheets into excel, script your process of hooking them together. First you want to clean each one individually and then use some of the techniques below to stitch them together into one big file
...
```{r}
################################################
#ababase<-read.csv('ababase.csv',header=T)   ###we'll deal with this later
myebird<-read.csv('C:/Users/avanderlaar/Downloads/ebird_boone.csv')

###now this data set has a lot going on, it has a lot of superfulous data, mix of data (x and numbers)
str(myebird)

###basically the first thing i do when uploading a data set is set everything to lower case
#tolower() makes this quite easy
?tolower
...
##in this case the periods in the column names are not a problem
...
test<-myebird[!(grepl(bad,myebird$commonname)),]
### we see this doesn't work, it's only searching for the first argument not subsequent arguements

##we could do this
test<-myebird[grepl('sp\\.',myebird$commonname),]   ##'why '\\.'## simply because we need to search special characters
test<-myebird[grepl('hybird',myebird$commonname),]  

##but theres no need we'll wrap it
####we'll wrap it in an sapply
badnames<-sapply(bad,grepl,myebird$commonname)
###what exactly did we just do?

View(badnames)
apply(badnames,1,sum)
test<-myebird[apply(badnames,1,sum)==0,]
unique(test$commonname)


###and that seems to take care of the easy ones,
myebird<-myebird[apply(badnames,1,sum)==0,]
##but what do we do with color morphs and subspecies?
### well these all have parenthesis in common, perhaps we can just subset by that?

test<-myebird[grepl('\\(',myebird$commonname),]

###this basically works, but it takes out species I know I do not have any other entries of, like Attwaters Greater Prairie Chicken
#lets look at it a different way. Since these are species we do want to count, lets just lump them into their species categories

newnames<-substr(myebird$commonname,1,(regexpr('\\(',myebird$commonname)-2))

###now this works ok, but we now have blank spots where there shouldn't be changes
myebird$commonname<-as.character(myebird$commonname)   ###this is necessary because it's a factor
myebird$commonname[nchar(newnames)>0]<-newnames[nchar(newnames)>0]
myebird$commonname<-as.factor(myebird$commonname)   ##its better to convert this back to a factor

View(myebird)
###there is likely other ways to do this, but this works
...
colnames(myebird)
myebird<-subset(myebird,select=-c(scientificname,taxonomicorder,allobsreported,numberofobservers,breedingcode,speciescomments,checklistcomments))


#####now lets further fix some of this, for some future analysis
#For example I don't really am interested in my life list, this data set doesn't explicitly state anything
#about my life birds. So let's make a life list.
...
?strptime  ###for a list of format characters
...
####################################################
###so we want to make a data set that creates a life list and tells me when, where, and what bird each life bird was
#lets start by creating a true date time column. That way we basically know to the hour when we first saw a bird
```
...
lifer<-aggregate(myebird,by=list(myebird$commonname),function(x){x[[1]]})
lifer<-lifer[order(lifer$timez),]

lifer<-arrange(lifer,timez)   ###dplyr equivalent

lifer$lifebird<-1:nrow(lifer)

View(lifer)
lifer[600,]
###
ggplot(lifer,aes(timez,lifebird)) + geom_line()
ggplot(lifer,aes(longitude,latitude)) +geom_point()
ggplot(myebird,aes(longitude,latitude)) +geom_point()
...
myebird$effortcount <- myebird$count / myebird$distancetraveledkm
...
myebird <- myebird[!(is.na(myebird$effortcount)),]

# tadah! NAs are gone. 

# there are functions which will do this, but when you only want to remove NAs from a particular 
# column it is often easier to phrase it this way. 
...
spread(gdat[,1:4], year, lifeexp) %>% head()
spread(gdat[,c(1:3,5)], year, pop) %>% head()
spread(gdat[,c(1:3,6)],year, gdppercap) %>% head()

head(gdat)

```

Long format has an identification column (vegetation point id #) and two other columns, variable (the name of the variable "water depth, site, etc") and one for value (the value for that vegetation point and that variable)

The advantage of long format data is it is easier to summarize and reform into graphs, tables, and to perform analysis. 
...
#creating an arbitary id variable to split back out. This could be anything, a date, a site name and point number, really anything
...
# I realize this is an arbitary id value that we are trying to seperate but this is super useful for lots of things
...
one of the big ways that we want to manipulate our data is to create tables so that we can look at our data in different ways. There are ways of creating publication ready tables in R, we're not going to cover them today, as they also involve Latex, and that is another day (atleast) to itself
