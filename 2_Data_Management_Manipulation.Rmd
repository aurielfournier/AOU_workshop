...
test<- myebird %>% filter(!grepl('\\(',myebird$commonname))
...
myebird$commonname <- gsub("\\s*\\([^\\)]+\\)","",as.character(myebird$commonname))
...
lifer <- myebird %>%  mutate(commonname=first(commonname))

lifer<- lifer %>% arrange(timez)
...
there are functions which will do this, na.omit() to the entire data frame but when you only want to remove NAs from a particular column it is often easier to phrase it this way. 
...
gdat %>% select(-pop,-gdppercap) %>% spread(year, lifeexp) %>% head()
